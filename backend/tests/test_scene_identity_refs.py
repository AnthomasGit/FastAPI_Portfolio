...
async def test_two_canonical_chars_land_on_ref_nodes(db_session, project, scene, tmp_path, monkeypatch):
    ...
    assert meta["workflow"] == "image_minimax_h3_ref"
    assert meta["ref_count"] == 2
    # Deterministic by name: Aldous -> slot 0 (node 137), Bex -> slot 1 (node 139).
    assert workflow["137"]["inputs"]["image"] == f"{a.id}_canon.png"
    assert workflow["139"]["inputs"]["image"] == f"{b.id}_canon.png"
    # Unused ref slots pruned.
    assert "141" not in workflow
...
async def test_char_without_canonical_is_skipped(db_session, project, scene, tmp_path, monkeypatch):
    ...
    # Only the one with a canonical image contributes; no error.
...
async def test_more_chars_than_slots_are_capped(db_session, project, scene, tmp_path, monkeypatch, caplog):
    ...
    # 10 canonical characters, but the workflow maps 9 ref slots.
    for i in range(10):
    ...
    assert meta["ref_count"] == 9  # capped to the 9 slots
