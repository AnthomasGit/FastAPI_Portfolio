# Vendored from ComfyUI's comfy/background_removal/birefnet.py (core ComfyUI,
# not a custom node), adapted to run standalone on CPU without ComfyUI's
# server/model-management framework. Module structure, class names, and
# nn.Module attribute names are kept byte-identical to the original so the
# birefnet.safetensors state dict loads unchanged (safetensors matches by
# key name — renaming an attribute here would break loading).
#
# What actually changed, and why each is safe for a plain "load full weights,
# run once, on CPU" inference (no ComfyUI VRAM-patching/quantization needed):
#   - `operations.{Linear,Conv2d,BatchNorm2d,LayerNorm}` is now passed in as a
#     plain torch.nn shim (see inference.py) instead of comfy.ops's classes —
#     those are thin subclasses adding VRAM-offload hooks we don't need here.
#   - `comfy.ops.cast_to_input(x, ref)` -> `x.to(device=ref.device, dtype=ref.dtype)`
#     (that's all cast_to_input does when there's no mixed-precision/offload state).
#   - `comfy.ops.cast_bias_weight`/`uncast_bias_weight` (VRAM-offload-aware
#     weight access) -> direct `.weight`/`.bias` attribute reads, since our
#     Conv2d is a plain nn.Conv2d with no offload state to cast around.
#   - `optimized_attention_for_device(..., small_input=True)` picks a backend
#     from several ComfyUI-specific attention implementations; on CPU it
#     resolves to a plain PyTorch path anyway, so this calls
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        relative_position_bias = relative_position_bias.permute(2, 0, 1).contiguous().to(device=attn.device, dtype=attn.dtype)  # nH, Wh*Ww, Wh*Ww
...
class DeformableConv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        weight, bias = self.regular_conv.weight, self.regular_conv.bias
